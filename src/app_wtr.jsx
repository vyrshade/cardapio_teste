...
const useWtrFetcher = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = fetchCollection(claims.merchantId, 'order', ftr,
      (data) => {
...
export default function Garçom() {
  ...
  const handleFecharConta = async (checkoutId, consumo, tableId, row) => {
    ...
    /* 
        const generateShortUID = (length = 20) => {
    ...
    updateDocumentsBatch(claims.merchantId, 'order', documentsIds, order);
