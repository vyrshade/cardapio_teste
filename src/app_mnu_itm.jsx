...
const useMnuFetcher = () => {
  ...
  useEffect(() => {
    ...
    const unsStatus = fetchCollection(params.merchantId, 'order_status', null,
      (data) => {
        //atualiza o estado
        setData((state) => ({
          ...state,
...
export default function CardapioItem() {
  ...
  const { error, fetching, user } = useMnuFetcher();
  ...
  const handleSelectedItem = async () => {
    ...
    const status = data.mnu.order_status.find(s => s.id === 'CAR');
    ...
    const order = {
      ...
      customer: {
        uid: user.uid,
        name: user?.displayName || 'Consumidor'
      },
      status: status.id,
