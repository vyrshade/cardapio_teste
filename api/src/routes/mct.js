...
router.post('/newUser', async (req, res) => {
  ...
  if (!validarCNPJ(user.cnpj)) {
    return res.status(500).json({ status: 'error', message: 'CNPJ inválido!' });
  }
  ...
  if (!merchantSnapshot.empty) {
    return res.status(500).json({ status: 'error', message: 'Já existe uma conta com esse CNPJ!' });
  }
...
router.post('/recPwd', async (req, res) => {
  ...
  try {
    ...
    if (merchantSnapshot.empty) {
      return res.status(500).json({ status: 'error', message: 'CNPJ não existe!' });
    }
